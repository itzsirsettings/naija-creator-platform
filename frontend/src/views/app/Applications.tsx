            ...
            <div key={a.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  <Avatar className="size-10 border border-border">
                    ...
                    <AvatarFallback className="bg-[#1A24B8]/10 text-[#1A24B8] font-semibold">{initials(a.creator?.name || "?")}</AvatarFallback>
                  </Avatar>
              ...
              {a.status === "PENDING" ? (
                <div className="mt-3 flex gap-2">
                  <button onClick={() => respond(a.id, "ACCEPTED")} className="rounded-lg bg-[#1A24B8] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#0A0F7A]">Accept</button>
